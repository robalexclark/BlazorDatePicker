...
namespace BlazorDatePicker
{
	public class DatePickerComponentBase : PickerComponentBase
	{
		...
		[Parameter] public DateTime? SelectedDate
		{
			get => _selectedDate;
			set
			{
		...
		[Parameter] public IEnumerable<DateTime> EnabledDates { get; set; }
		/// <summary>Days of the week that cannot be selected.</summary>
		[Parameter] public IEnumerable<DayOfWeek> DaysOfWeekDisabled { get; set; }
		/// <summary>Inclusive date intervals that cannot be selected.</summary>
		[Parameter] public IEnumerable<(DateTime start, DateTime end)> DisabledDateIntervals { get; set; }
		...
		protected override void OnInitialized()
		{
			...
			{
				MonthToDisplay = new DateTime(SelectedDate.Value.Year, SelectedDate.Value.Month, 1);
			}
			else
			{
		...
		internal string DayOfWeekAbbreviation(DayOfWeek DayOfWeek) => FormatProvider.DateTimeFormat.GetShortestDayName(DayOfWeek);
		...
		internal void SetSelectedDate(DateTime selectedDate)
		{
		...
		internal IEnumerable<DateTime> Days { get; private set; }
		...
		internal void SetSelectedMonth(int month)
		{
			...
			var selectedDate = new DateTime(MonthToDisplay.Year, month, 1);
		...
		internal void SetDisplayMonth(int month)
		{
			...
			MonthToDisplay = new DateTime(MonthToDisplay.Year, month, 1);
		}
		...
		internal void NextYear(EventArgs eventArgs)
		{
			...
			MonthToDisplay = MonthToDisplay.AddYears(1);
			RenderMonths();
			_onUpdated();
		}
		...
		internal void SetDisplayYear(int year)
		{
			...
			MonthToDisplay = new DateTime(year, MonthToDisplay.Month, 1);
		}
		...
		internal void NextDecade(EventArgs eventArgs)
		{
			...
			MonthToDisplay = MonthToDisplay.AddYears(10);
			RenderYears();
			_onUpdated();
		}
		...
		internal void NextCentury(EventArgs eventArgs)
		{
			...
			MonthToDisplay = MonthToDisplay.AddYears(100);
			RenderDecades();
			_onUpdated();
		}
		...
		internal void DayClicked(EventArgs eventArgs, DateTime date)
		{
		...
		internal Func<DateTime, bool> DaysEnabledFunction { get; set; }
		...
		internal DateTime MonthToDisplay { get; private set; }
